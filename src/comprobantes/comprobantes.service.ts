...
import { EPCpteProductos } from '../envio_pack/entities/cpte-productos-ep.entity';

@Injectable()
export class ComprobanteService {
    constructor(
        @InjectRepository(Comprobante, 'cnnPres')
        public compRepository: Repository<Comprobante>,
        @InjectRepository(Comprobante_items, 'cnnPres')
        public compItemRepository: Repository<Comprobante_items>,
        @InjectRepository(Producto_Movimiento, 'cnnPres')
        public compProdMovRepository: Repository<Producto_Movimiento>,
        @InjectRepository(CC_Movimientos, 'cnnPres')
        public compCCMovRepository: Repository<CC_Movimientos>,
        @InjectRepository(CC_Movimientos_Campos, 'cnnPres')
        public compCCMovCamposRepository: Repository<CC_Movimientos_Campos>,
        @InjectRepository(Despacho, 'cnnPres')
        public compDespachoRepository: Repository<Despacho>,
        @InjectRepository(EPCpte, 'cnnEP')
        public EPRepository: Repository<EPCpte>,
        @InjectRepository(EPCpteDespacho, 'cnnEP')
        public EPDespachoRepository: Repository<EPCpteDespacho>,
        @InjectRepository(EPCpteProductos, 'cnnEP')
        public EPProductosRepository: Repository<EPCpteProductos>,
    ) {
    }
    ...
    async getOne(id: number): Promise<any> {
        ...
        if (pComprobante) {
            ...
            for (const pMov of pCCMovimientos) {
                const pCCMovimientosCampos =
                    await this.compCCMovCamposRepository.findOne({
                        where: {
    ...
    async editarComprobante(editarComprobante: Comprobante) {
        try {
            ...
            const cleanComprobante =
                this.cleanComprobanteForUpdate(editarComprobante);
            await this.compRepository.update(
            ...
            for (const item of editarComprobante.comprobanteItems) {
                ...
                if (exists) {
                    ...
                    await this.compItemRepository.update(
                        { idComprobanteItem: item.idComprobanteItem },
                        item,
                    );
                } else {
            ...
            const newComprobanteItemIds = new Set(
                editarComprobante.comprobanteItems.map(
                    (item) => item.idComprobanteItem,
                ),
            );
            // Filtro los items que no están en comprobanteItems
            const itemsToDelete = actualItems.filter(
                (item) => !newComprobanteItemIds.has(item.idComprobanteItem),
            );
            ...
            const listaIdCompItems = actualItems.map(
                (item) => item.idComprobanteItem,
            );
            ...
            for (const prodMov of editarComprobante.productos_movimiento) {
                const exists = actualMovs.some(
                    //Aca busco si en la lista de items del comprobante, esta el item de la base de datos
                    (actMov) =>
                        actMov.idProductoMovimiento === prodMov.idProductoMovimiento,
                );

                if (exists) {
                    //Si esta lo actualizo, si no esta ya se creo antes con el comprobanteItem
                    await this.compProdMovRepository.update(
                        { idProductoMovimiento: prodMov.idProductoMovimiento },
                        prodMov,
                    );
                }
            }
            //Elimino los productos_movimientos que ya no estan mas
            const newProdMovIds = new Set(
                editarComprobante.productos_movimiento.map(
                    (item) => item.idProductoMovimiento,
                ),
            );
            const movsToDelete = actualMovs.filter(
                (item) => !newProdMovIds.has(item.idProductoMovimiento),
            );
            if (movsToDelete.length > 0) {
                const idsToDelete = movsToDelete.map(
                    (item) => item.idProductoMovimiento,
                );
            ...
            if (editarComprobante.despacho) {
                await this.compDespachoRepository.update(
                    { idDespacho: editarComprobante.despacho.idDespacho },
                    editarComprobante.despacho,
                );
            }
            ...
            const actualCCMovIds = new Set(
                actualCCMovimientos.map((mov) => mov.idCCMovimiento),
            );
            const newCCMovimientos = editarComprobante.CCMovimientos.filter(
                (mov) => !actualCCMovIds.has(mov.idCCMovimiento),
            );
            ...
            for (const item of actualCCMovimientos) {
                ...
                if (exists) {
                    ...
                    const ccMovModified = editarComprobante.CCMovimientos.find(
                        (mov) => mov.idCCMovimiento === item.idCCMovimiento,
                    );
                    const cleanCCMovModified =
                        this.cleanCCMovimientoForUpdate(ccMovModified);
                    await this.compCCMovRepository.update(
                        { idCCMovimiento: item.idCCMovimiento },
                        cleanCCMovModified,
                    );
                    if (ccMovModified.CCMovimientoCampo) {
                        await this.compCCMovCamposRepository.update(
                            { idCCMovimiento: item.idCCMovimiento },
                            ccMovModified.CCMovimientoCampo,
                        );
                    }
                } else {
                    //Lo elimino
                    await this.compCCMovRepository.delete({
                        idCCMovimiento: item.idCCMovimiento,
                    });
                    await this.compCCMovCamposRepository.delete({
                        idCCMovimiento: item.idCCMovimiento,
                    });
                }
            }
            for (const newCCMov of newCCMovimientos) {
                ...
                const pNewCCMov =
                    await this.compCCMovRepository.create(cleanCCMovimiento);
                ...
                if (newCCMov.CCMovimientoCampo) {
                    ...
                    const pNewCCMovCampo = await this.compCCMovCamposRepository.create(
                        newCCMov.CCMovimientoCampo,
                    );
    ...
    private cleanComprobanteForUpdate(
        comprobante: Partial<Comprobante>,
    ): Partial<Comprobante> {
        //Esta funcion le saca las propiedades que no se actualizan en la base de datos para evitar errores
        const {
            comprobanteItems,
    ...
    private cleanCCMovimientoForUpdate(
        CC_Movimientos: Partial<CC_Movimientos>,
    ): Partial<CC_Movimientos> {
    ...
    async create(newComprobante: Comprobante) {
        try {
            // EntityManager para la transacción
            return await this.compRepository.manager.transaction(
                async (entityManager: EntityManager) => {
                    ...
                    for (const item of newComprobante.comprobanteItems) {
                        ...
                        const pNewProdMov = entityManager.create(
                            Producto_Movimiento,
                            prodMov,
                        );
                    ...
                    if (newComprobante.despacho) {
                        ...
                        const pNewDespacho = entityManager.create(
                            Despacho,
                            newComprobante.despacho,
                        );
                        pNewDespacho.idComprobante = idComprobante;
                        await entityManager.save(pNewDespacho);
                        idDespacho = pNewDespacho.idDespacho;
                    }
                    ...
                    for (const ccMov of newComprobante.CCMovimientos) {
                        ...
                        if (ccMov.CCMovimientoCampo) {
                            ...
                            const CCMovCampo: CC_Movimientos_Campos =
                                new CC_Movimientos_Campos();
                            CCMovCampo.idCCMovimiento = ccMov.idCCMovimiento;
                            CCMovCampo.bancoCCMovimientoCampo =
                                ccMov.CCMovimientoCampo.bancoCCMovimientoCampo;
                            CCMovCampo.numeroOperacionCCMovimientoCampo =
                                ccMov.CCMovimientoCampo.numeroOperacionCCMovimientoCampo;
                            CCMovCampo.numeroChequeCCMovimientoCampo =
                                ccMov.CCMovimientoCampo.numeroChequeCCMovimientoCampo;
                            CCMovCampo.fechaCobroCCMovimientoCampo =
                                ccMov.CCMovimientoCampo.fechaCobroCCMovimientoCampo;
                            CCMovCampo.fechaEmisionCCMovimientoCampo =
                                ccMov.CCMovimientoCampo.fechaEmisionCCMovimientoCampo;
                            CCMovCampo.tipoCambioCCMovimientoCampo =
                                ccMov.CCMovimientoCampo.tipoCambioCCMovimientoCampo;

                            const newCCMovCampo = entityManager.create(
                    ...
                    if (newComprobante.envio_pack) {
                        const listaProdEp: EPCpteProductos[] = [];
                        //Es ep, lo cargo en ep
                        //TO DO: Agregar transaction
                        const pNewEPCpte = await this.EPRepository.create(
                            newComprobante.envio_pack,
                        );
                        pNewEPCpte.idComprobante = idComprobante;
                        pNewEPCpte.idDespacho = idDespacho;
                        await this.EPRepository.save(pNewEPCpte);

                        if (newComprobante.envio_pack.cpte_despacho) {
                            //grabo en cpte_despachos
                            const pNewEPCpteDespacho = await this.EPDespachoRepository.create(
                                newComprobante.envio_pack.cpte_despacho,
                            );
                            pNewEPCpteDespacho.idComprobante = idComprobante;
                            pNewEPCpteDespacho.idDespacho = idDespacho;
                            await this.EPDespachoRepository.save(pNewEPCpteDespacho);
                        }

                        for (const epProducto of newComprobante.envio_pack.cpte_productos) {
                            //Primero agrupo los skus que son iguales en los items, y sumo las cantidades
                            const p = listaProdEp.find(
                                (item) => item.codigoProducto === epProducto.codigoProducto,
                            );
                            if (p) {
                                p.cantidad += epProducto.cantidad;
                            } else {
                                listaProdEp.push(epProducto);
                            }
                        }
                        for (const itemEp of listaProdEp) {
                            //Grabo en cptes_productos
                            const pNewEPCProducto =
                                await this.EPProductosRepository.create(itemEp);
                            pNewEPCProducto.idComprobante = idComprobante;
                            await this.EPProductosRepository.save(pNewEPCProducto);
                        }
                    }
